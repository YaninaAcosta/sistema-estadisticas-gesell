/**
 * Importa balnearios desde TSV/CSV o JSON.
 ...
 *
 * Uso:
 *   node scripts/import-balnearios.js [ruta]
 * Por defecto: data/balnearios-importar.json (o .tsv)
 *
 * JSON: array de { localidad, prestador, direccion?, telefono_fijo?, whatsapp? }
 * TSV: Localidad  Prestadores  Web  e-mail  Dirección  Teléfono fijo  WhatsApp
 */
...
const pathArg = process.argv[2] || join(__dirname, '..', 'data', 'balnearios-importar.json');
...
function cleanPhone(s) {
  ...
  const t = s.trim();
  if (/no posee|no tienen|no llamar/i.test(t)) return null;
  return t || null;
}
...
if (pathArg.endsWith('.json')) {
  ...
  rows = lines.slice(1).map((line) => {
    ...
    return {
      ...
      direccion: col(cols, 4).trim() || null,
...
const { data: existentes, error: errList } = await supabase.from('balnearios').select('localidad, prestador');
if (errList) {
  console.error('Error al listar balnearios:', errList.message);
  process.exit(1);
}
...
console.log(`Balnearios en archivo: ${rows.length}, sin duplicados: ${unicos.length}, ya existían: ${unicos.length - aInsertar.length}, a insertar: ${aInsertar.length}`);
...
for (let i = 0; i < aInsertar.length; i += BATCH) {
  ...
  const { error } = await supabase.from('balnearios').insert(chunk);
  if (error) {
...
console.log(`Importados ${insertados} balnearios. Localidades: VG→Villa Gesell, MDLP→Mar de las Pampas, MA→Mar Azul, LG→Las Gaviotas, CM→Colonia Marina.`);
process.exit(0);
